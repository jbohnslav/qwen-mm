...
fn read_split(cfg: &Json<'_>) -> Result<SplitPretok> {
    ...
    let native = SplitPretok::native(pattern.clone(), behavior, invert)?;
    // rc.2 constructed every regex without a backend, including unrecognized
    // Qwen3.5 patterns. Keep native grammars fast and compile the fallback.
    #[cfg(feature = "fancy-regex")]
    if native.grammar().is_none() {
        return SplitPretok::new(pattern, behavior, invert)?.canonicalized_for_pipeline();
    }
    native.canonicalized_for_pipeline()
}
